		...
		public function upload_files(string $key = '') :string|array
		{
			...
			if(!empty($this->files())) {
				...
				if($get_one && empty($this->files()[$key])) {
					$this->upload_errors['name'] = 'Archivos no encontrados';
					return '';
				}
				...
				foreach($this->files() as $key => $file_arr) {
					...
					if($file_arr['error'] > 0) {
						...
						$this->upload_errors     = "Se produjo un error con los archivos: " . $file_arr['name'];
						continue;
					}
					
					if(!in_array($file_arr['type'], $this->upload_file_types)) {
						
						$this->upload_errors[] = "Tipo de archivo invalido: " . $file_arr['name'];
						continue;
					}
					
					if($file_arr['size'] > ($this->upload_max_size * 1024 * 1024)) {
						
						$this->upload_errors[] = "Archivo demasiado grande: " . $file_arr['name'];
						continue;
					}
