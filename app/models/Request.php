		...
		public function upload_files(string $key = ''):string|array
		{
			...
			foreach($this->files() as $ikey => $file_arr){
				...
				if($file_arr['error'] > 0){
					...
					$this->upload_errors[$ikey] = "Se produjo un error con los archivos: " . $file_arr['name'];
					continue;
				}
				
				if(!in_array($file_arr['type'],$this->upload_file_types)){
					
					$this->upload_errors[$ikey] = "Tipo de archivo invalido: " . $file_arr['name'];
					continue;
				}
				
				if($file_arr['size'] > ($this->upload_max_size * 1024 * 1024)){
					
					$this->upload_errors[$ikey] = "Archivo demasiado grande: " . $file_arr['name'];
					continue;
				}
